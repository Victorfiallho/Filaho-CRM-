-- Fialho CRM — Supabase schema (Phase 1 scaffold, not yet connected)
--
-- Mirrors Filaho-CRM-/schema.md and the shape FialhoDB keeps in memory today
-- (app.js). IDs stay `text` so the values already generated by uid() in the
-- app (e.g. "cust_mrkv7uwz_3plj9f") can be migrated as-is later, with no
-- remapping of foreign keys.
--
-- This file is not run against any project yet. RLS policies are included
-- but commented out — uncomment them once real Supabase Auth + company_members
-- rows exist (Phase 2), otherwise every table would reject all access.
